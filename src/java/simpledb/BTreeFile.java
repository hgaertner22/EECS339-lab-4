...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		// base case is when we are on leaf node
		if(pid.pgcateg() == BTreePageId.LEAF){
			BTreeLeafPage resultLeaf = (BTreeLeafPage) this.getPage(tid, dirtypages, pid, perm);
			return resultLeaf;
		}

		// otherwise, we have an internal page
		BTreeInternalPage internalPage = (BTreeInternalPage) this.getPage(tid, dirtypages, pid, perm);
		BTreeEntry nextEntry = null;

		// now, we must iterate across the field values on the internal page
		Iterator<BTreeEntry> entryIterator = internalPage.iterator();
		while(entryIterator.hasNext()){
			nextEntry = entryIterator.next();


			// if null, we must traverse left on the tree
			if(f == null) {
				return this.findLeafPage(tid, dirtypages, nextEntry.getLeftChild(), Permissions.READ_ONLY, f);
			}

			// find the first entry such that the field is less than the entry
			// in which case, we need the child on the left of the entry
			if(f.compare(Op.LESS_THAN_OR_EQ, nextEntry.getKey())){
				return this.findLeafPage(tid, dirtypages, nextEntry.getLeftChild(), Permissions.READ_ONLY, f);
			}

		}

		// if we made it to the end, there is no entry greater than the value, and we must return final right node
		BTreeLeafPage resultLeaf = this.findLeafPage(tid, dirtypages, nextEntry.getRightChild(), Permissions.READ_ONLY, f);
		return resultLeaf;
	}
